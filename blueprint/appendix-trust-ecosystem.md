...
sequenceDiagram
    ...
    W->>TL: 3. Fetch Trusted List or List of Trusted Entities
    TL-->>W: 4. Trusted List or List of Trusted Entities Response
    ...
    Note over W: 8. Validate WRPRC in the request (RPRC_17)<br/>- Verify WRPRC Provider in Trusted List or List of Trusted Entities<br/>- If intermediated: WRPRC shows this intermediary (RPRC_04, RPRC_17a)<br/>- If absent or invalid: warn User at approval
...
- **Wallet and RP side:** Wallet Units verify RPs via the **WRPRC in the presentation request** and Access CA Trusted Lists or List of Trusted Entities (see [Relying Party Registration & Access Certificates](#relying-party-registration--access-certificates)). Registry APIs remain for publication. PID Providers and Attestation Providers verify Wallet Providers against the Wallet Provider Trusted List before issuing credentials.
